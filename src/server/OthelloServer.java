...
//                }
    ...
    public boolean checkHang(int x0, int y0) throws IOException {
        boolean check = false;
        if((y0+1)==y) {
        } else if (matrandanh[x0][y0+1] == 1) {
            int k =y0+1;
            int i = y0+1;
            while(i<y) {
                if(matrandanh[x0][i] == 1) {
                    i++;
                } else {
                    k = i;
                    break;
    ...
    public boolean checkCot(int x0, int y0) throws IOException {
        ...
        if((x0+1)==x) {
         ...
         else if(matrandanh[x0+1][y0] == 1) {
            int k =x0+1;
            int i = x0+1;
            while(i<x) {
                if(matrandanh[i][y0] == 1) {
                    i++;
                } else {
                    k = i;
                    break;
    ...
    public boolean checkCheoXuong(int x0, int y0) throws IOException {
        ...
        if((x0+1)==x || (y0+1) == y) {
        ...
        else if(matrandanh[x0+1][y0+1] == 1) {
            int k =1;
            int i = 1;
            while((i+y0<y) && (x0+i)<x) {
                if(matrandanh[x0+i][y0+i] == 1) {
                    i++;
                } else {
                    k = i;
                    break;
            ...
            else if( (matrandanh[x0+k][y0+k] == 2)) {
                check = true;
                oos.writeObject("doiCheoXuong1,"+String.valueOf(x0)+","+String.valueOf(k)+"," +String.valueOf(y0));
                for(int t=1; t<k; t++) {
                    matrandanh[x0+t][y0+t] = 2;
                    bt[x0+t][y0+t].setBackground(Color.BLACK);
        ...
        if((x0-1)==-1 || (y0-1) == -1) {
            ...
            } else if(matrandanh[x0-q][y0-q] == 2) {
                check =true;
